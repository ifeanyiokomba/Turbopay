...
export async function POST(req: Request) {
  ...
  try {
    const result = await stripeService.createCheckoutSession(user.id, parsed.data.amountNaira);
    return json({ data: result });
  } catch (e: any) {
    if (e instanceof ServiceError) return errorJson(e.message, e.status, e.code);
    return errorJson(e.message || "Failed to create checkout session", 500);
  }
}
