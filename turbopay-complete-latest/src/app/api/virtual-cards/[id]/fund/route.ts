...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const limited = await rateLimit(req, { key: "card-fund", limit: 10, windowMs: 60_000, scope: "user", userId: user.id });
  ...
  try {
    const result = await enhancedCards.fundCard(id, user.id, parsed.data.amountKobo);
    // Fire-and-forget in-app notification. Never throws, never blocks.
    notify.sendInApp({
      userId: user.id, type: "TRANSACTION", title: "Card Funded",
      message: `₦${(parsed.data.amountKobo / 100).toLocaleString()} added to card · New balance: ₦${(result.newCardBalanceKobo / 100).toLocaleString()}`,
