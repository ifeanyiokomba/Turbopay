...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const limited = await rateLimit(req, { key: "savings-withdraw", limit: 10, windowMs: 60_000, scope: "user", userId: user.id });
  ...
  try { await savings.withdraw(id, user.id, parsed.data.amountKobo); return json({ data: { ok: true } }); }
